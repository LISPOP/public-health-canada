...
## Province FSA mismatch

As part of the process of merging our data with the health region file in order to access the covid19 case counts, it was discovered that 
`r nrow(subset(full, province_fsa_bad==1))`  cases feature reported provinces that do not match the reported FSA. These have been flagged in the variable `province_fsa_bad`. For the moment, the assigned health region and rural variable assume that the reported FSA is correct and the reported province is a mistake. 

```{r eval=F, echo=F}
names(full)
full %>% 
  filter(province_fsa_bad==1) %>% 
  select(CID, FSA, province_original, province)
```

# Rural
Matt can you add in a section here describing how we added in the rural variable?
